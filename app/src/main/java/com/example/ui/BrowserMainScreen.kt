...
fun BrowserMainScreen(
    ...
    val webPermissionsState = rememberMultiplePermissionsState(
        permissions = listOf(
    ...
    LaunchedEffect(Unit) {
        if (!webPermissionsState.allPermissionsGranted) {
            webPermissionsState.launchMultiplePermissionRequest()
        }
    }
    ...
    MyApplicationTheme(
        ...
        Scaffold(
            ...
            bottomBar = {
                if (!uiState.isFullscreen && !uiState.isOmniboxFocused) {
                    BottomNavBar(
