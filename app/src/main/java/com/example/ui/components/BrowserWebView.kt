    ...
    override fun onInterceptTouchEvent(ev: MotionEvent): Boolean {
        when (ev.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                // Jangan aktifkan jika gerakan horizontal (misal geser carousel/tab/swipe) atau ke atas
                if (dy <= 0 || dx > dy * 0.75f) {
                    return false
                }

                // Harus melewati batas tarikan vertikal yang disengaja
                if (dy < touchSlop) {
                    return false
                }
            }
...
fun BrowserWebView(
    ...
    // Update User Agent & Viewport if Desktop Mode changes
