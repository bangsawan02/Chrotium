...
fun BrowserWebView(
    ...
    val webView = remember(tab.id) {
        BackgroundPlayWebView(context).apply {
            ...
            if (com.example.engine.WebConfig.isGoogleAuthUrl(tab.url)) {
                settings.userAgentString = com.example.engine.WebConfig.getGoogleAuthUserAgent(tab.isDesktopMode)
            ...
            webViewClient = object : WebViewClient() {
                ...
                override fun onPageStarted(view: WebView?, url: String?, favicon: Bitmap?) {
                    ...
                    // Dynamic User-Agent adjustment for Google Accounts & OAuth compatibility
                    if (isAuth) {
                        view?.settings?.userAgentString = com.example.engine.WebConfig.getGoogleAuthUserAgent(tab.isDesktopMode)
                    } else if (tab.isDesktopMode) {
                        view?.settings?.userAgentString = com.example.engine.WebConfig.getDesktopUserAgent()
                    } else {
                        view?.settings?.userAgentString = com.example.engine.WebConfig.getMobileUserAgent()
                    }
                ...
                override fun onPageFinished(view: WebView?, url: String?) {
                    ...
                    // Ensure cookie sync after auth redirect
                ...
                override fun shouldOverrideUrlLoading(view: WebView?, request: WebResourceRequest?): Boolean {
                    ...
                    if (com.example.engine.WebConfig.isGoogleAuthUrl(urlStr)) {
                        view?.settings?.userAgentString = com.example.engine.WebConfig.getGoogleAuthUserAgent(tab.isDesktopMode)
                    }
