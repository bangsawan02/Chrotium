...
class SuggestionEngine(private val database: AppDatabase) {
    ...
    private fun fetchSearchQuerySuggestions(query: String): List<String> {
        ...
        try {
            ...
            if (ddgConn.responseCode == HttpURLConnection.HTTP_OK) {
                val responseText = ddgConn.inputStream.bufferedReader().use { it.readText() }
            ...
            try { ddgConn?.disconnect() } catch (e: Exception) {}
        }

        if (list.isNotEmpty()) return list

        // 2. Fallback ke Google Suggestions jika DuckDuckGo kosong
        var googleConn: HttpURLConnection? = null
        try {
            val fallbackUrl = URL("https://suggestqueries.google.com/complete/search?client=chrome&q=$encodedQuery")
            googleConn = (fallbackUrl.openConnection() as HttpURLConnection).apply {
                requestMethod = "GET"
                connectTimeout = 1500
                readTimeout = 1500
            }
            if (googleConn.responseCode == HttpURLConnection.HTTP_OK) {
                val responseText = googleConn.inputStream.bufferedReader().use { it.readText() }
                val jsonArray = JSONArray(responseText)
                if (jsonArray.length() > 1) {
                    val suggestionsArray = jsonArray.getJSONArray(1)
                    for (i in 0 until suggestionsArray.length().coerceAtMost(6)) {
            ...
            try { googleConn?.disconnect() } catch (e: Exception) {}
        }
